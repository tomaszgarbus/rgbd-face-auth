...
from common.tools import image_size
...
def vap_load_train_subject(subject_no, img_no):
    path_depth = DB_LOCATION + 'www.vap.aau.dk/files/%d/0%02d_%d_d.depth' % (subject_no+1, 1+img_no/3, 1+img_no%3)
    path_color = DB_LOCATION + 'www.vap.aau.dk/files/%d/0%02d_%d_c.png' % (subject_no+1, 1+img_no/3, 1+img_no%3)
...
def ias_load_train_subject(subject_no, img_no):
    path_depth = DB_LOCATION + 'ias_lab_rgbd/files/training/0%02d_%02d.depth' % (subject_no, img_no)
    path_color = DB_LOCATION + 'ias_lab_rgbd/files/training/0%02d_%02d.png' % (subject_no, img_no)
    # Load depth and color photo
    color_photo = tools.load_color_image_from_file(path_color)
    depth_photo = tools.load_depth_photo(path_depth)
    # Resize to common size
...
def superface_load_train_subject(subject_no, img_no):
    path_color = DB_LOCATION + 'superface_dataset/files/subject%03d/rgbFrame/frameRGB%d.png' % (subject_no+1, img_no+1)
    path_depth = DB_LOCATION + 'superface_dataset/files/subject%03d/depthFrame/frameD%d.depth' % (subject_no+1, img_no+1)
...
class DBHelper:
    ...
    def imgs_per_subject(self, subject_no):
        if self.db_name == 'www.vap.aau.dk':
            ...
            color_path = DB_LOCATION + 'superface_dataset/files/subject%03d/rgbFrame' % (subject_no+1)
            depth_path = DB_LOCATION + 'superface_dataset/files/subject%03d/depthFrame' % (subject_no+1)
