\chapter{Skin recognition}
    ...
    \section{RGB skin detection simulation}
        ...
        However, condictions described in paper gave us too many false positives (what is understandable),
        so we decided to improve decision proces. If we will name vector $(Y, Cb, Cr)$ for pixel $p = (R, G, B)$
        mark of $p$, then we calculated for every pixel in image it's mark.
        After that we were taking a few marks of pixels frome central part of image and
        sorting them by euclidian norm.
        Heuristically middle element of new table is skin picture.
        At the end, we consider every pixel as skin pixel if and only if
        it's mark was close enough to midle mark from sorted list.
